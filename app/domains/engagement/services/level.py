...
# 特权说明：全部为**展示类**权益，且均已在前端真实生效（不含需要改造 AI 链路的额度承诺）。
# 落地点（改动本列表时必须同步这几处，否则又会出现「承诺了但看不到」的条目）：
#   · "基础徽标"     → 顶栏 Lv.N 徽标本身（App.vue .lv-badge）；
#   · "称号专属配色" → logic/level.js 的 levelHue() 按 lv 逐级变化（Lv1 青绿 150° → Lv20 紫 290°），
#                      经 CSS 变量 --lv-h 下发到顶栏徽标 / 等级页 hero / 进度条 / 特权条；
#   · "头像框「X」"  → levelFrameTier() 从 Lv2 起每 2 级解锁 1 档（共 10 档，对应 10 个框名），
#                      经 --lv-tier 控制 .lv-frame 的边框厚度与光晕强度；
#   · "成就墙展示位" → 徽章墙顶部渲染等级展示卡（BadgesView .lv-showcase，含跳转等级页）。
_PERKS = (
