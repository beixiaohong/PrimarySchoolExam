...
| **[小说站模块说明](./小说站模块说明.md)** | **TXT 导入 + 自动分章 / 下滑流式加载** | 产品 / 前端 / 后端 | 独立入口 `/novel`（独立 Vue 应用）、3 张表（`db_novels` 等）、TXT 编码探测与分章判定规则、`/read` 下滑加载契约、读者端 8 + 后台 8 端点、部署与测试 |
...
| 治理总纲 | [优化方案-代码与功能优化-2026Q4.md](./优化方案-代码与功能优化-2026Q4.md) | **取代**旧《优化建议书》：基于真实代码审计的 P0/P1/P2 代码优化 + 功能优化 + 待新增功能清单（含 RBAC/钻石锁两条红线、索引/N+1、上帝文件拆分、路线图） |
| 治理总纲（历史快照） | [优化建议书.md](./优化建议书.md) | 2026-08-12 历史审计快照，部分条目已落地/漂移，仅作演进对照 |
