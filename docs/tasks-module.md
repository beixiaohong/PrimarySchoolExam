...
> 智学学堂「每日任务」子系统 —— 强制+可选双轨制（默认 3+3，可自定义）、自定义任务、补签卡、完成确认。
...
### 双轨制（默认 3 强制 + 3 可选，均可家长自定义）

- **强制任务**：每科默认 1 条固定（数学练习 / 古诗文背诵 / 学单词），三科全完成 = 当天全勤。
  家长可在「任务设置」**按科整体替换**——把该科换成 `MANDATORY_CHOICES` 内的任意组合
  （每条 code 一行，可含默认项本身，数量 0~N）；该科未配置/配置为空时回退默认项。
  注意：默认项并非"不可换的固定项"，2b3dd4c 起语义为「每科完整 code 列表整体替换默认」。
- **可选任务**：家长可配置可选列表（`settings.optional`，全部生成）；未配置时系统每日从
  任务池确定性随机抽 3 条。全部可选完成奖励 1 张补签卡。
...
**生命周期自管理**：家长自定义任务行（`task_code=custom:N`，subject 可能为"其他"）
**不受** `POST /api/tasks/settings` 保存清理影响——保存设置只清理标准强制/可选行，
custom 行由 `/custom-task` 更新/删除接口在变更时同步移除今日未完成行。
（bc0fd8d 修复：此前 settings 清理循环会触碰 subject="其他" 的 custom 行，触发
`_get_mandatory_codes` KeyError → 保存任务设置 500；且会误删三科 custom 行。）
...
| `bc0fd8d` | 修复保存设置 500 + 误删 custom 行（`_get_mandatory_codes` 未知学科防御；settings 清理循环跳过 custom:/未知学科行；测试对齐「完整替换默认」语义） |
