...
const appOptions = {
  data() {
    return {
      ...
      rechargeCopied: false,   // 支付弹窗复制按钮状态（商城复用）
      // 首页
      ...
      // 网课（2026-08-20：系统配置默认展示；家长单独配置 parentCourses/courseForm 已移至 logic/parent.js）
  ...
  computed: {
    ...
    // 家长端「每日任务设置」概览 mandatorySummary 已移至 logic/parent.js（parentComputed）
  },
  ...
  methods: {
    ...
    goTab(t) {
      this.tab = t;
      if (t === 'home') { this.loadRewards(); this.loadRewardTimeline(); this.loadParentMsgs(); this.loadNotices(); this.loadDailyTasks(); }
